            ...
            if (idx_lon < idx_be) {
                ...
                if (idx_cheo != -1 && idx_cheo < idx_lon) {
                    ...
                    if (idx_lon < idx_be && idx_be < idx_cheo) {
                        ...
                        LogUtils.debug("dinh dang khi j day chua parse dc");
                    }
                }
            } else {
            ...
            if (contentsHtml.length() > idx_cut) {
                ...
                if (idx_br_fst == -1 || idx_br_lst <= idx_br_fst) {
                    ...
                    if ((idx_img_lst > -1 && idx_img_lst < idx_br_lst) || idx_img_fst > idx_br_fst) {
                        ...
                        iCuted = (idx + 10);
                        LogUtils.debug("gia tri cut 11:" + iCuted);
...
//                            Tool.debug("lon hon:");
                        ...
                        LogUtils.debug("gia tri cut 22:" + iCuted);
                        if (iCuted > contentsHtml.length()) {
//                            Tool.debug("lon hon:");
                            iCuted = contentsHtml.length();
                        }
...
//                        Tool.debug("33--fstText:" + (idx_cut + idx_lst + 5) + "--lstText:" + (idx_cut + idx_lst + 5));
                        LogUtils.debug("gia tri cut 33:" + (lstText.indexOf("<br/>") + 5));
...
//                        Tool.debug("22--<img/> ko nam trong doan");
                        LogUtils.debug("gia tri cut 44:" + (idx_cut + idx_br_lst + 5));
                        iCuted = (idx_cut + idx_br_lst + 5);
//                        Tool.debug("gia tri cut 22:" + iCuted);
                        if (iCuted > contentsHtml.length()) {
//                            Tool.debug("lon hon:");
...
//                        sReturn = contentsHtml.substring(0, iCuted) + "--chen vao giua--" + web.htmlParse(contentsHtml.substring(iCuted),idx_cut);
            ...
            LogUtils.debug("loi htmlParse(): " + e.getMessage());
        }
        return sReturn;
    }
    ...
    public String invailHtml(String sHtml) {
        ...
        try {
            ...
            LogUtils.debug("iBOpen-->" + iBOpen + "--iBEnd-->" + iBEnd + "--iIOpen-->" + iIOpen + "--iIEnd-->" + iIEnd);
            if (iBOpen != iBEnd && iIOpen == iIEnd) {           //thieu the <b> con the <i> du
                LogUtils.debug("11111111---->");
...
//                    Tool.debug("mo it hon dong--->bbbb");
                ...
                LogUtils.debug("555555555---->");
                sReturn = sHtml;
            }
        } catch (Exception e) {
            LogUtils.debug("loi invailHtml0---web.java--->" + e.getMessage());
        }
        return sReturn;
    }
...
//        String complie = "</b>";
    ...
    public String addLinkDownTest(String content, String folder, long id, int stt, int tong) {
        ...
        try {
            ...
            LogUtils.debug("loi Web.java--addLinkDown(): " + e.getMessage());
        }
        return sreturn;
    }
    ...
    public static String FindTag(String sStr) {
        ...
        int j = 0;
        for (int i = 0; i < Tag.length; i++) {
            if (MyString.convert2NoSign(Tag[i]).matches("[A-Z\\u0110][a-zA-Z]*")) {
                LogUtils.debug("Tag " + i + ": OK");
                if ((i == 0 || i == 1) && i - j <= 1) {
                    sTag = sTag + " " + Tag[i];
                } else if (i - j == 1) {
                    sTag = sTag + " " + Tag[i];
                ...
                j = i;
                if (MyString.convert2NoSign(Tag[i]).matches("[A-Z\\u0110][A-Z]*")) {
                    sTag = sTag.trim();
                }
                flag = true;
            } else if (i == 1) {
